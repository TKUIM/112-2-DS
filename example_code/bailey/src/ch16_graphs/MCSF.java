...
public class MCSF {
    ...
    public static void main(String[] args)
    {
        ...
        while (!s.eof()) {
            String here = s.readString(); // 接收邊頂點here標記
            if(here.equals("END"))
            {
                break;
            }
            String there = s.readString();// 接收邊頂點there標記
            Integer weight = s.readInt(); // 接收邊權重weight數字
            s.skipWhite();
            g.add(here);  // 圖加頂點here
            g.add(there); // 圖加頂點there
            g.addEdge(here,there,weight); // 圖加邊，兩頂點 here,there，權重 weight
        }
        System.out.println("In original graph there are "+g.size()+" vertices"+
                           " and "+g.edgeCount()+" edges.");

        // 計算最小成本生成森林
        System.out.println("mcsf...");
        mcsf(g);
        System.out.println();
        ...
        while (ei.hasNext())
        {
            ...
            if (e.isVisited()) {
               weight += e.label(); // 累加森林所有邊的成本
               edges++;  // 累加森林所有邊的個數
               System.out.println(e); // 列印森林的邊
            }
        }
        System.out.println("Total weight is "+weight+" over "+edges+" edges.");
    }

    // 利用堆積，針對無向圖g，建立最小成本生成森林
    // 生成森林的樹邊有拜訪過標識
    ...
    //       a minimum spanning forest
    {
        PriorityQueue<ComparableEdge<String,Integer>> q =
          new SkewHeap<>(); // for ranking edges
        g.reset();              // clear marks
        ...
        while (ei.hasNext())
        {
            q.add(new ComparableEdge<>(ei.next()));
        }
        ...
        while (!q.isEmpty())
        {
            ...
            g.visit(e.here());
            g.visit(e.there());
            Edge<String,Integer> ge = g.getEdge(e.here(),e.there());
            g.visitEdge(ge);
        }
    }
}
