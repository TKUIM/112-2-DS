/*
*  MCST.java
*     利用最小成本生成樹找出圖的所有元件，統計元件邊數及權重和
*
* > java MCST
input edge (use END to stop): here there weight
...
Albany--120--Boston
Trenton--4200--Athens
Bangkok--650--KualaLumpur
Dover--250--Boston
Harrisburg--100--Trenton
Sacramento--550--Phoenix
Sacramento--450--SaltLakeCity
Boston--130--Montpelier
KualaLumpur--4700--Athens
SaltLakeCity--1500--Harrisburg
...
package ch16_graphs;
import structure5.ComparableEdge;
import structure5.Edge;
import structure5.Graph;
import structure5.GraphListUndirected;
...
public class MCST {
    ...
    {
        // 建立空的無向圖
        Graph<String,Integer> g = new GraphListUndirected<>();
        ...
        System.out.println("input edge (use END to stop): here there weight");
        for (hereTown = input.readString();  // 接收邊頂點here標記
             !hereTown.equals("END");
             hereTown = input.readString()) {
        
            thereTown = input.readString();  // 接收邊頂點there標記
            roadName = input.readString();   // 接收路名
            length = input.readInt();        // 接收路長
        
            g.add(hereTown);  // 圖加頂點here
            g.add(thereTown); // 圖加頂點there

            // for getting from hereTown to thereTown
            // 圖加邊，兩頂點 here,there，長度 length
        ...
        OrderedStructure<String> results = new OrderedList<>(); // 建立字串排序清單
        
        // 不斷找出生成樹元件，將其頂點刪除，直到圖沒有頂點為止
        while (g.size() != 0)
        {
            // 計算最小成本生成樹
            System.out.println("\nmcst...");
            ...
            while (ei.hasNext())
            {
                ...
                if (e.isVisited())
                {
                    // 列印本生成樹元件的邊，格式: here頂點標記 -- 權重 -- there頂點標記 
            ...
            totalWeight += weight;  // 加總權重
            totalCount += count;  // 加總邊數
            
            // 收集生成樹頂點標記清單
            List<String> l = new SinglyLinkedList<>();  // 建立空清單
            for (String vtx : g) {
                if (g.isVisited(vtx))
                    l.add(vtx);  // 清單加入生成樹頂點標記
            }
